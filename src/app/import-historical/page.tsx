...
import { processHistoricalCSV, HistoricalCSVRow, ImportResult } from '@/actions/historicalImport';
import { Archive, CheckCircle, AlertTriangle, Loader2 } from 'lucide-react';
import Link from 'next/link';

export default function HistoricalImportPage() {
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<HistoricalCSVRow[]>([]);
  const [allRows, setAllRows] = useState<HistoricalCSVRow[]>([]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      Papa.parse<HistoricalCSVRow>(selectedFile, {
        header: true,
        skipEmptyLines: true,
        complete: (results) => {
  ...
  const handleImport = async () => {
    ...
    setResult(null);
    try {
      const res = await processHistoricalCSV(allRows);
      setResult(res);
    } catch (error) {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-white mb-2">Import Historical Data</h2>
        <p className="text-zinc-400">Upload older journey logs to backfill your customer history.</p>
      </div>
      ...
      {!result ? (
        ...
        <div className="space-y-6">
          <div className="glass-panel p-8 rounded-2xl text-center relative overflow-hidden">
            ...
            <p className="text-zinc-400 relative z-10">Your historical data has been successfully processed.</p>
